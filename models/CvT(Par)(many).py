...
def train_and_save_model(freq, labels_dict, proc_dict_scaled, images, valid_dict, count):
    ...
    # 檢查並建立資料夾
    weight_folder = os.path.join(script_dir, '../Result/Weight/Images & Parameters')
    record_folder = os.path.join(script_dir, '../Result/Records/Images & Parameters')
    ...
    # 保存模型權重
    model.save_weights(os.path.join(weight_folder, f'cvt_model_weights_{freq}_{time}_{projection_method}_cls{cls_token_switch}.h5'))

    # 初始化 DataFrame 以存儲記錄
    records = pd.DataFrame(model.history.history)
    records.insert(0, 'epoch', range(1, len(records) + 1))
    records.to_excel(os.path.join(record_folder, f'cvt_records_{freq}_{time}_{projection_method}_cls{cls_token_switch}.xlsx'), index=False)
