...
def save_predictions_to_excel(predictions, actual, train_num, test_num, r2, mse, mae, freq):
    ...
    # 檢查並建立資料夾
    Excel_folder = os.path.join(script_dir, '../Result/Excel/Parameters')
    
    if not os.path.exists(Excel_folder):
        os.makedirs(Excel_folder)

    # 儲存至Excel
    excel_path = os.path.join(Excel_folder, f'Predictions_Metrics_{freq}_{time}.xlsx')
...
def test_and_save_results(freq, labels_dict, proc_dict_scaled, valid_dict, count):
    ...
    # 載入模型權重
    model.load_weights(os.path.join(script_dir, f'../Result/Weight/Parameters/Vit_model_weights_{freq}_{time}.h5'))
    ...
    plt.savefig(os.path.join(Plots_folder, f'Vit_R^2_{freq}_{time}.png'))
    plt.clf()
    ...
    plt.legend()
    plt.savefig(os.path.join(Plots_folder, f'Vit_Actual_vs_Predicted_{freq}_{time}.png'))
    plt.clf()
