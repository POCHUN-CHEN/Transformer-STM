...
# 確定文件的相對路徑
Circle_test_data_path = os.path.join(script_dir, '../Excel/Processed_Circle_test.xlsx')

# 讀取Excel文件中的標簽數據
excel_data = pd.read_excel(Circle_test_data_path)
...
def train_and_save_model(freq, labels_dict, images, valid_dict, count):
    ...
    # 檢查並建立資料夾
    weight_folder = os.path.join(script_dir, '../Result/Weight/Images')
    record_folder = os.path.join(script_dir, '../Result/Records/Images')
    ...
    # 保存模型權重
    model.save_weights(os.path.join(weight_folder, f'cvt_model_weights_{freq}_{projection_method}_cls{cls_token_switch}.h5'))

    # 初始化 DataFrame 以存儲記錄
    records = pd.DataFrame(model.history.history)
    records.insert(0, 'epoch', range(1, len(records) + 1))
    records.to_excel(os.path.join(record_folder, f'cvt_records_{freq}_{projection_method}_cls{cls_token_switch}.xlsx'), index=False)
