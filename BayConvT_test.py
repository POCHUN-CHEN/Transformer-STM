...
for freq in frequencies:
    ...
    # 載入模型權重
    model.load_weights(f'Weight/Images/bayesian_conv_transformer_model_weights_{freq}.h5')
    ...
    plt.savefig(f'Plots/Images/R^2_{freq}.png')  # 儲存圖片
    # plt.show()
    ...
    plt.legend()  # 顯示圖例
    plt.savefig(f'Plots/Images/Actual_vs_Predicted_{freq}.png')  # 儲存圖片
    # plt.show()
    plt.clf() # 清空畫布（plt.show()內建有）
